...
DELETE /api/payments/{id}        → Soft-flag as deleted (status=archived)
...
def update_payment(payment_id: int, data: PaymentUpdate):
    ...
    # Guard: NaN check for amount
    if data.amount is not None and data.amount != data.amount:  # NaN check
        raise HTTPException(400, "amount must be a valid number")
...
def refund_payment(payment_id: int):
    ...
    if p["status"] != "completed":
        raise HTTPException(
            400,
            f"Only completed payments can be refunded. Current status: {p['status']}"
        )
