...
const TEAM_COLOR = {
  "Payments Team":  "#3b82f6",
  "QA Payments":    "#f59e0b",
  "Platform Team":  "#22c55e",
  "Release Manager":"#8b5cf6",
...
export default function PRTab({ reloadSignal, onSelect, onDataChange }) {
  ...
  // Memoize filtered list so it's not recomputed on every keystroke render
  const filtered = useMemo(() => {
    return prs.filter(pr => {
      const matchFilter =
        filter === "all" ||
        pr.risk_level?.toLowerCase() === filter ||
        pr.event_type === filter;
  ...
  return (
    <div style={{ animation: "fadeIn 0.2s ease" }}>
      ...
      <div style={{ display: "flex", justifyContent: "space-between", alignItems: "center", marginBottom: 14, flexWrap: "wrap", gap: 10 }}>
        <div>
          <h2 style={{ color: "#e6edf3", fontSize: 16, fontWeight: 700 }}>
            🤖 Agent Zed — PR Intelligence
          </h2>
          <p style={{ fontSize: 12, color: "#6b7280", marginTop: 3 }}>
            GitHub webhooks trigger 4 AI agents automatically on every PR open / approve / merge
          </p>
        </div>
        <button
          onClick={() => setShowTrigger(true)}
          style={{ background: "linear-gradient(135deg,#1d4ed8,#4c1d95)", border: "none", color: "#fff", padding: "9px 18px", borderRadius: 8, cursor: "pointer", fontSize: 13, fontWeight: 600 }}
        >
          ⚡ Run Agent Manually
        </button>
      </div>
      ...
      <div style={{ display: "grid", gridTemplateColumns: "repeat(4,1fr)", gap: 10, marginBottom: 16 }}>
        {[
          ...
          ["🎯", "Impact Analysis",    "Maps real files → teams → dependency risk",          "#f59e0b"],
          ["🚀", "Release Intelligence","Checks calendar conflicts, stakeholders",            "#ef4444"],
          ["🧠", "RAG Knowledge",      "Captures senior patterns, grows smarter over time",  "#10b981"],
...
function PRRow({ pr, onClick, onDelete, onRerun, isActioning }) {
  ...
  return (
    <div
      onClick={onClick}
      style={{ display: "grid", gridTemplateColumns: "44px 1fr 130px 160px 140px 90px 80px", alignItems: "center", padding: "12px 16px", cursor: "pointer", borderBottom: "1px solid #161b22", transition: "background 0.12s", opacity: isActioning ? 0.5 : 1 }}
      onMouseEnter={e => e.currentTarget.style.background = "#0f1724"}
      onMouseLeave={e => e.currentTarget.style.background = "transparent"}
    >
      <div style={{ fontSize: 20, textAlign: "center" }}>
        {EVENT_ICON[pr.event_type] || "📋"}
      </div>
      ...
      <div style={{ minWidth: 0 }}>
        ...
        <div style={{ display: "flex", gap: 10, fontSize: 11, color: "#6b7280", flexWrap: "wrap" }}>
          ...
          <span style={{ background: "#1e3a5f", color: "#93c5fd", padding: "1px 6px", borderRadius: 4, fontSize: 10, fontFamily: "monospace" }}>
            {pr.base_branch || "main"}
          </span>
        </div>
      </div>
      ...
      <div>
        ...
        {(pr.files_changed || []).length > 2 && (
          <div style={{ fontSize: 10, color: "#6b7280" }}>
            +{pr.files_changed.length - 2} more
          </div>
        )}
      </div>
      ...
      <div>
        {(pr.impacted_teams || []).slice(0, 2).map(t => (
          <span key={t} style={{ display: "inline-block", fontSize: 10, padding: "2px 7px", borderRadius: 10, background: `${TEAM_COLOR[t] || "#9ca3af"}18`, color: TEAM_COLOR[t] || "#9ca3af", border: `1px solid ${TEAM_COLOR[t] || "#9ca3af"}30`, marginBottom: 3, marginRight: 3, whiteSpace: "nowrap" }}>
            {t}
          </span>
        ))}
        {(pr.impacted_teams || []).length > 2 && (
          <div style={{ fontSize: 10, color: "#6b7280" }}>
            +{pr.impacted_teams.length - 2}
          </div>
        )}
      ...
      <div>
        <span style={{ display: "inline-block", fontSize: 11, fontWeight: 700, padding: "3px 10px", borderRadius: 20, background: `${rc}18`, color: rc, border: `1px solid ${rc}40` }}>
          {risk}
        </span>
      ...
      <div style={{ display: "flex", flexDirection: "column", gap: 4 }} onClick={e => e.stopPropagation()}>
        <button
          onClick={onRerun}
          style={{ background: "#1e3a5f", border: "1px solid #3b82f640", color: "#3b82f6", padding: "4px 6px", borderRadius: 6, cursor: "pointer", fontSize: 12, width: "100%" }}
          title="Re-run agents"
        >
          {isActioning ? "⏳" : "🔄"}
        </button>
        <button
          onClick={onDelete}
          style={{ background: "#2d1010", border: "1px solid #ef444440", color: "#ef4444", padding: "4px 6px", borderRadius: 6, cursor: "pointer", fontSize: 12, width: "100%" }}
          title="Delete"
        >
          🗑
        </button>
...
function ManualTriggerModal({ onClose, onSuccess }) {
  ...
  async function handleRun() {
    if (!form.pr_id.trim() || !form.title.trim()) {
      setError("PR ID and Title are required");
      return;
    }
    if (!form.files.trim()) {
    ...
    try {
      await runAgentZed({
        ...
        author:        form.author.trim() || "manual",
        repo:          form.repo.trim(),
    ...
    <>
      ...
      <div style={{ position: "fixed", top: "50%", left: "50%", transform: "translate(-50%,-50%)", background: "#0d1117", border: "1px solid #30363d", borderRadius: 14, padding: "26px 28px", width: "min(560px,95vw)", zIndex: 200, animation: "fadeIn 0.2s ease", maxHeight: "90vh", overflowY: "auto" }}>
        ...
        <p style={{ color: "#6b7280", fontSize: 12, marginBottom: 18 }}>
          Triggers all 4 AI agents: PR Review → Impact Analysis → Release Intelligence → RAG Knowledge
        </p>

        <div style={{ display: "grid", gridTemplateColumns: "1fr 1fr", gap: 10 }}>
          <F label="PR ID *"  value={form.pr_id}  onChange={v => set("pr_id", v)}  placeholder="PR-121" />
          <F label="Author"   value={form.author}  onChange={v => set("author", v)} placeholder="your-github-username" />
        </div>
        <F label="PR Title *"    value={form.title}       onChange={v => set("title", v)}       placeholder="Add payment validation" />
        <F label="Repository"    value={form.repo}        onChange={v => set("repo", v)}         placeholder="org/repo-name" />
        <div style={{ display: "grid", gridTemplateColumns: "1fr 1fr", gap: 10 }}>
          <F label="Base Branch"  value={form.base_branch} onChange={v => set("base_branch", v)} placeholder="main" />
          <F label="Head Branch"  value={form.head_branch} onChange={v => set("head_branch", v)} placeholder="feature/my-change" />
        </div>
        <div style={{ marginBottom: 12 }}>
          ...
          <textarea
            ...
            rows={4}
            style={inp}
            placeholder={"routers/payments.py\ndatabase.py\ntests/test_payments.py"}
          />
        </div>
        <div style={{ marginBottom: 14 }}>
          <Lbl>PR Description (optional)</Lbl>
          <textarea
            value={form.body}
        ...
        {error && (
          <div style={{ background: "#1a0a0a", border: "1px solid #ef444440", borderRadius: 6, padding: "8px 12px", fontSize: 12, color: "#f87171", marginBottom: 12 }}>
            ❌ {error}
          </div>
        )}
        ...
        <div style={{ display: "flex", gap: 10, justifyContent: "flex-end" }}>
          <button
            onClick={onClose}
            style={{ background: "transparent", border: "1px solid #30363d", color: "#8b949e", padding: "9px 18px", borderRadius: 8, cursor: "pointer", fontSize: 13 }}
          >
            Cancel
          </button>
          <button
            ...
            disabled={loading}
            style={{
              background: loading ? "#0f2a1a" : "linear-gradient(135deg,#1d4ed8,#4c1d95)",
              border:  loading ? "1px solid #22c55e40" : "none",
          ...
          >
            {loading ? (
              <>
                <span style={{ display: "inline-block", width: 13, height: 13, border: "2px solid #22c55e", borderTopColor: "transparent", borderRadius: "50%", animation: "spin 0.8s linear infinite" }} />
                Analyzing PR... (10–15s)
              </>
            ) : "⚡ Run All Agents"}
          </button>
...
function Skeleton() {
  return (
    <>
      {[1, 2, 3].map(i => (
        <div key={i} style={{ height: 72, margin: "1px 0", background: "linear-gradient(90deg,#161b22 25%,#21262d 50%,#161b22 75%)", backgroundSize: "400px 100%", animation: "shimmer 1.4s infinite" }} />
      ))}
    </>
  );
}
...
function F({ label, value, onChange, placeholder }) {
  return (
    <div style={{ marginBottom: 12 }}>
      <Lbl>{label}</Lbl>
      <input value={value} onChange={e => onChange(e.target.value)} placeholder={placeholder} style={{ ...inp, resize: undefined }} />
    </div>
  );
}
...
const inp = {
  width: "100%", background: "#070b14", border: "1px solid #30363d",
