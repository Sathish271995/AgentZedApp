...
async def github_webhook(
    ...
    print(f"\n🔔 GitHub event: {x_github_event} / {action} — PR#{pr_data['pr_id']}")
    return await _pipeline(pr_data)


def _extract_files(data: dict) -> list:
    """
    GitHub webhook JSON does NOT contain files[].
    Calls GitHub API to get the real changed files.
    Requires GITHUB_TOKEN in .env (repo read scope).
    """
    ...
    if pr_num and repo and token:
        try:
            ...
            resp = req.get(url, headers=hdrs, timeout=10)
            if resp.status_code == 200:
                return [f["filename"] for f in resp.json()]
            print(f"  ⚠️  GitHub API {resp.status_code} for PR #{pr_num}")
        except Exception as e:
            print(f"  ⚠️  GitHub API error: {e}")

    # Fallback: guess from PR title if token missing
...
def _ai_reply(comment: str, pr: dict) -> str:
    if not _is_configured():
        return "Agent Zed: Configure Azure OpenAI in .env for AI responses."
    try:
        client = _get_client()
        ...
        return f"Agent Zed error: {str(e)}"