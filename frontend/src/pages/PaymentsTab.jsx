...
export default function PaymentsTab({ onDataChange }) {
  ...
  const load = async () => {
    setLoading(true);
    try {
      const qs = [filter && `status=${filter}`, custFilter && `customer_id=${custFilter}`].filter(Boolean).join("&");
      const res = await fetchPayments(qs ? `?${qs}` : "");
      setPayments(res.payments || []);
    } catch(e) { setError(e.message); }
    setLoading(false);
  };

  useEffect(() => { load(); }, [filter, custFilter]);

  const handleDelete = async (id) => {
    if (!confirm("Delete this payment?")) return;
    try { await deletePayment(id); load(); onDataChange(); }
    catch(e) { alert(e.message); }
  };

  const handleRefund = async (id) => {
    if (!confirm("Refund this payment?")) return;
    try { await refundPayment(id); load(); onDataChange(); }
    catch(e) { alert(e.message); }
  };

  const handleStatus = async (id, status) => {
    try { await updatePayment(id, { status }); load(); onDataChange(); }
    catch(e) { alert(e.message); }
  };
    ...
    <div style={{ animation:"fadeIn 0.2s ease" }}>
      ...
      <div style={{ background:"#0d1117", border:"1px solid #21262d", borderRadius:10, overflow:"hidden" }}>
        <div style={{ display:"grid", gridTemplateColumns:"60px 1fr 100px 80px 110px 100px 180px", padding:"10px 16px", borderBottom:"1px solid #21262d", fontSize:11, color:"#6b7280", fontWeight:700, textTransform:"uppercase", letterSpacing:"0.06em" }}>
          <span>ID</span><span>Description / Customer</span><span>Amount</span><span>Currency</span><span>Status</span><span>Created</span><span>Actions</span>
        </div>
        ...
        {loading ? <Skeleton /> : payments.length === 0 ? (
          ...
          <div key={p.id} style={{
            display:"grid", gridTemplateColumns:"60px 1fr 100px 80px 110px 100px 180px",
            alignItems:"center", padding:"12px 16px", borderBottom:"1px solid #161b22",
            ...
            <div>
              <span style={{
                ...
                border:`1px solid ${STATUS_COLOR[p.status]||"#6b7280"}40`,
            ...
            <div style={{ display:"flex", gap:4, flexWrap:"wrap" }}>
              ...
              {/* Refund */}
              {p.status === "completed" && (
                <button onClick={() => handleRefund(p.id)} style={actBtn("#1a0a2e","#8b5cf6")} title="Refund">↩</button>
              )}
              {/* Edit */}
              <button onClick={() => { setEditItem(p); setShowForm(true); }} style={actBtn("#1e3a5f","#3b82f6")} title="Edit">✏</button>
              {/* Delete */}
              <button onClick={() => handleDelete(p.id)} style={actBtn("#2d1010","#ef4444")} title="Delete">🗑</button>
            </div>
          </div>
        ))}
      </div>
...
function PaymentForm({ item, onClose, onSaved }) {
  ...
  async function handleSubmit() {
    if (!form.amount || !form.customer_id) { setError("Amount and Customer ID are required"); return; }
    setLoading(true); setError("");
    try {
      if (isEdit) await updatePayment(item.id, form);
    ...
    <>
      ...
      <div style={{ position:"fixed",top:"50%",left:"50%",transform:"translate(-50%,-50%)",background:"#0d1117",border:"1px solid #30363d",borderRadius:14,padding:"26px 28px",width:"min(480px,95vw)",zIndex:200,animation:"fadeIn 0.2s ease" }}>
        ...
        <div style={{ display:"grid",gridTemplateColumns:"1fr 1fr",gap:12,marginBottom:12 }}>
          <div>
            <Label>Amount ($) *</Label>
            <input type="number" step="0.01" min="0.01" value={form.amount} onChange={e=>set("amount",parseFloat(e.target.value))} style={inputStyle} placeholder="250.00" />
          </div>
          <div>
...
// ── Helpers ───────────────────────────────────────────────────────────────────
function Skeleton() {
  return <>{[1,2,3,4].map(i=><div key={i} style={{ height:60,margin:"1px 0",background:"linear-gradient(90deg,#161b22 25%,#21262d 50%,#161b22 75%)",backgroundSize:"400px 100%",animation:"shimmer 1.4s infinite" }} />)}</>;
}
